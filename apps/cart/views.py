    ...
    if request.method == 'POST':
        ...
        if quantity > product.stock:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'status': 'warning',
                    'message': f'Lo sentimos, solo hay {product.stock} unidades disponibles de {product.nombre}.'
                })
            else:
                messages.warning(request, f'Lo sentimos, solo hay {product.stock} unidades disponibles de {product.nombre}.', extra_tags='toast')
        ...
        try:
            ...
            if cart_item.cantidad + quantity > product.stock:
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return JsonResponse({
                        'status': 'warning',
                        'message': f'No puedes agregar más unidades. Stock disponible: {product.stock}'
                    })
                else:
                    messages.warning(request, f'No puedes agregar más unidades. Stock disponible: {product.stock}', extra_tags='toast')
            else:
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return JsonResponse({
                        'status': 'success',
                        'message': f'Se actualizó la cantidad de {product.nombre} en tu carrito a {cart_item.cantidad}.',
                        'cart_count': cart.cantidad_productos
                    })
                else:
                    messages.success(request, f'Se actualizó la cantidad de {product.nombre} en tu carrito a {cart_item.cantidad}.', extra_tags='toast')
        except ItemCarrito.DoesNotExist:
            # Si no existe, crear nuevo item
            cart_item = ItemCarrito(carrito=cart, producto=product, cantidad=quantity)
            cart_item.save()
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'status': 'success',
                ...
                messages.success(request, f'Se agregó {product.nombre} a tu carrito.', extra_tags='toast')
        
        logger.debug(f'Producto agregado/actualizado en carrito: {product.nombre}, cantidad: {quantity}')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                messages.success(request, f'{product_name} eliminado del carrito.', extra_tags='toast')
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                messages.error(request, 'Producto no encontrado en el carrito.', extra_tags='toast')
...
def update_cart(request, product_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if quantity > product.stock:
                messages.warning(request, f'Solo hay {product.stock} unidades disponibles.', extra_tags='toast')
            ...
            logger.debug(f'Carrito actualizado: {product.nombre}, cantidad={quantity}')
            
            # No mostramos mensaje de actualización para evitar mensajes intrusivos
            ...
            messages.error(request, 'Error al actualizar el carrito.', extra_tags='toast')
...
def clear_cart(request):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, 'El carrito ha sido vaciado.', extra_tags='toast')
            logger.debug(f'Carrito vaciado para usuario: {request.user.username}')
