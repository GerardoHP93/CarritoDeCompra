...
class DireccionEnvioForm(forms.Form):
    ...
    codigo_postal = forms.CharField(label='Código postal', max_length=10, 
                                  widget=forms.TextInput(attrs={'class': 'form-control'}))
    pais = forms.CharField(label='País', max_length=100, initial='México', 
                         widget=forms.TextInput(attrs={'class': 'form-control'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and user.is_authenticated:
            ...
            if direcciones:
                self.fields['direccion_guardada'].choices = [(d.id, f"{d.calle} {d.numero_exterior}, {d.colonia}, {d.ciudad}") for d in direcciones]
                self.fields['direccion_guardada'].choices.insert(0, ('', 'Selecciona una dirección'))
            else:
...
class MetodoPagoForm(forms.Form):
    ...
    def clean(self):
        ...
        if metodo_pago == 'tarjeta':
            campos_requeridos = ['numero_tarjeta', 'titular_tarjeta', 'fecha_expiracion', 'cvv', 'tipo_tarjeta']
